# site/

The public write-up: all 41 rounds, what each one asked, and what came through
the controls. One self-contained static page.

```
site/
  index.html      the whole site: markup, styles, and the round data inline
  og.jpg          1200x630 link preview (WhatsApp, iMessage, Slack, X, LinkedIn)
  favicon.svg     the fish sign (220), outline pulled from the font
  favicon-32.png  fallback for browsers that will not take an SVG icon
  apple-touch-icon.png  180x180 home-screen icon
  fonts/indus.ttf the Indus Script Font (PUA E0xx-E7xx), used to render real signs
```

Round summaries live in the `ROUNDS` array near the bottom of `index.html`.
Each entry is `{n, t, tag, q, a, stat?, signs?, file}`; `tag` drives the rail
colour, the outcome map in the hero, and the filter chips, whose counts
recompute themselves. `signs` holds HTML entities for PUA codepoints, which come
from `data/parsed/glyphs.json` (`glyph_id` to `unicode`).

## Link preview

`og.jpg` is the card scrapers show. The Open Graph and Twitter tags in `<head>`
point at `https://indus.primemodulus.com/og.jpg`; those URLs must stay absolute,
since WhatsApp and most scrapers will not resolve a relative one. Serving the
site from a different host means editing that base in the six tags that carry it.

The card is generated by `tools/make_og.py` (Pillow, system serif faces, and
`fonts/indus.ttf` for the signs): `python3 tools/make_og.py` from the repo root.
After changing it, re-scrape at developers.facebook.com/tools/debug/ — WhatsApp
reads Facebook's cache, so an old card can otherwise persist for days.

## Favicon

The tab icon is sign 220, the plain fish, with its outline lifted straight out
of `fonts/indus.ttf` so it matches what the page renders. `tools/make_favicon.py`
(fontTools for the SVG path, Pillow for the PNGs) regenerates all three:
`python3 tools/make_favicon.py` from the repo root. The SVG carries its own
`prefers-color-scheme` rule, so the fish turns pale on a dark tab strip.

## Local preview

```bash
cd site && python3 -m http.server 8712    # http://localhost:8712
```

## Deploy

`vercel.json` at the repo root points Vercel at this directory, so a plain
`vercel --prod` from the repo root ships it. No build step, no dependencies.

To deploy this folder on its own instead, run `vercel --prod` from inside
`site/` and delete the `outputDirectory` line from the root config.

## Font

`fonts/indus.ttf` is `sk_indus_script-webfont.ttf` from
[yajnadevam/indus-website](https://github.com/yajnadevam/indus-website), the
Indus Script Font the National Fund for Mohenjo-daro built from Parpola's sign
forms. It's copied here so the page stays self-contained.
